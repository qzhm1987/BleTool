...
//sec秒后停止
- (BabyBluetooth *(^)(int sec,BBNotDiscoverPeripheralsBlock block)) stop {
    
    
    return ^BabyBluetooth *(int sec,BBNotDiscoverPeripheralsBlock block) {
        ...
        self->timerBaby = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, queue);
        ...
        dispatch_source_set_timer(self->timerBaby, start, interval, 0);
        
        // 设置回调
        dispatch_source_set_event_handler(self->timerBaby, ^{
...
//                [timerForStop invalidate];
                ...
                // 取消定时器
                dispatch_cancel(self->timerBaby);
                self->timerBaby = nil;
            }
        });
        
        // 启动定时器
        dispatch_resume(self->timerBaby);
